...
FROM 'C:\Program Files\PostgreSQL\17\data\organizations-10000.csv'
...
-- Task 18 EXERCISE: Modify the query to include the average order amount instead of the total.
...
--Task 19 EXERCISE: Modify the query to show the number of orders instead of the total amount.
SELECT
    W.warehouse_name,
    O.order_count
FROM
...
-- Task 20 EXERCISE: Modify the query to find warehouses that processed orders for customers NOT from 'New York' or 'Dallas'.
SELECT
    warehouse_name
FROM
    Warehouses
...
-- Task 21 EXERCISE: Adjust the ranges to divide fulfillment percentages into quartiles (0-25%, 26-50%, etc.).
SELECT
    W.warehouse_name,
